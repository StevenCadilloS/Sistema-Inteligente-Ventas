...
Future<void> sembrarCatalogoDemo(AppDatabase db) async {
  final yaSembrado = await (db.select(
    db.productos,
  ...
  await db.batch((b) {
    b.insertAll(db.tiposProducto, _categorias());
    b.insertAll(db.productos, _productos(ahora));
    b.insertAll(db.estrategias, _estrategias());
  });
}

List<TiposProductoCompanion> _categorias() => [
  TiposProductoCompanion.insert(
    tipoProducto: 'T00001',
    nombreTipoProducto: 'Tecnologia',
  ),
  TiposProductoCompanion.insert(
    tipoProducto: 'T00002',
    nombreTipoProducto: 'Hogar',
  ),
  TiposProductoCompanion.insert(
    tipoProducto: 'T00003',
    nombreTipoProducto: 'Moda',
  ),
  TiposProductoCompanion.insert(
    tipoProducto: 'T00004',
    nombreTipoProducto: 'Belleza',
  ),
];

/// El `mostrado` va sembrado y variado a proposito: las reglas de `neutral`
/// (lo mas mostrado) y `sorpresa` (lo menos mostrado) necesitan historial para
/// ordenar distinto desde el primer arranque.
List<ProductosCompanion> _productos(int ahora) => [
  _producto(
    'P0000001',
    'Audifonos Bluetooth',
    'T00001',
...
List<EstrategiasCompanion> _estrategias() => [
  EstrategiasCompanion.insert(
    codEstrategia: 'E0000001',
    nombreEstrategia: 'Descuento directo',
  ),
  EstrategiasCompanion.insert(
    codEstrategia: 'E0000002',
    nombreEstrategia: 'Envio gratis',
  ),
  EstrategiasCompanion.insert(
  ...
  EstrategiasCompanion.insert(
    codEstrategia: 'E0000004',
    nombreEstrategia: 'Oferta relampago',
  ),
];
